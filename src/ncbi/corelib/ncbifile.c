...
size_t LIBCALL  Nlm_FileRead (void *ptr, size_t size, size_t n, FILE *stream)
{
...
size_t LIBCALL  Nlm_FileWrite (const void *ptr, size_t size, size_t n, FILE *stream)
{
	if ((n != 0) && ((Nlm_Int4)(SIZE_MAX / (Nlm_Int4) n) < (Nlm_Int4) size))
		ErrPostEx(SEV_WARNING,E_Programmer,0,"FileWrite:  size > SIZE_MAX");
	else if (ptr != NULL && stream != NULL)
	{
		size_t cnt = fwrite(ptr,size,n,stream);
		if (cnt != n)
...
*
*****************************************************************************/
char * LIBCALL  Nlm_FileGets (Nlm_CharPtr ptr, size_t size, FILE *fp)
{
