...
void * LIBCALL Nlm_MemNew PROTO((size_t size));
void * LIBCALL Nlm_MemGet PROTO((size_t size, unsigned int flags));
void * LIBCALL Nlm_MemMore PROTO((void *ptr, size_t size));
void * LIBCALL Nlm_MemExtend PROTO((void *ptr, size_t size, size_t oldsize));
void * LIBCALL Nlm_MemFree PROTO((void *ptr));
void * LIBCALL Nlm_MemCopy PROTO((void *dst, const void *src, size_t bytes));
void * LIBCALL Nlm_MemMove PROTO((void *dst, const void *src, size_t bytes));
void * LIBCALL Nlm_MemFill PROTO((void *ptr, int value, size_t bytes));
void * LIBCALL Nlm_MemDup PROTO((const void *orig, size_t size));

#if (defined(OS_MAC) || defined(WIN_MSWIN) || defined(MSC_VIRT))
Nlm_Handle  LIBCALL Nlm_HandNew PROTO((size_t size));
Nlm_Handle  LIBCALL Nlm_HandGet PROTO((size_t size, Nlm_Boolean clear_out));
Nlm_Handle  LIBCALL Nlm_HandMore PROTO((Nlm_Handle hnd, size_t size));
...
#ifdef WIN16
void *  LIBCALL win16_Malloc (size_t bytes);
void *  LIBCALL win16_Calloc (size_t items, size_t size);
void *  LIBCALL win16_Realloc (void *ptr, size_t size);
void    LIBCALL win16_Free (void *ptr);
#endif

#ifdef WIN_MAC
#ifdef USE_MAC_MEMORY
void *mac_Malloc  PROTO((size_t size));
void *mac_Calloc  PROTO((size_t nmemb, size_t size));
void *mac_Realloc PROTO((void *ptr, size_t size));
