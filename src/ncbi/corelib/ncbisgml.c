...
int LIBCALL  Nlm_SgmlLoadTable (void)
{
	...
	x = atoi(buf);
	sgml_entity = MemNew(x * sizeof(char*));
	sgml_ascii = MemNew(x * sizeof(char*));
	for (i = 0; i < x; i++)
	{
...
*
*****************************************************************************/
char * LIBCALL  Nlm_Sgml2Ascii (const char *sgml, char *ascii, size_t buflen)   /* length of ascii buffer */
{
	const char *from;
	char tbuf[40], *to, *tmp;
	int i, j;
...
*
*****************************************************************************/
size_t LIBCALL  Nlm_Sgml2AsciiLen (const char *sgml)
{
	const char *from;
	char tbuf[40], *tmp;
	int len = 0, i, j;
