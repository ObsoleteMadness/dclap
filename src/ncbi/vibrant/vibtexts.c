...
static void Nlm_GetPassword (Nlm_TexT t, Nlm_CharPtr passwd, size_t maxsize)
...

{
  ...
  Nlm_RecTToRectTool (&r, &rtool);
  h = TENew (&rtool, &rtool);
  Nlm_LoadTextData (t, h, NULL, NULL, FALSE, NULL, 0, FALSE, FALSE, FALSE, 1, NULL, NULL, NULL);
...

{
  ...
  Nlm_RecTToRectTool (&r, &rtool);
  h = TENew (&rtool, &rtool);
  Nlm_LoadTextData (t, h, NULL, NULL, FALSE, NULL, 0, FALSE, FALSE, TRUE, 1, NULL, NULL, tabProc);
  Nlm_SetDialogText ((Nlm_GraphiC) t, 0, local, FALSE);
  if (h != NULL) {
    TEAutoView (TRUE, h);
  }
...
#ifdef WIN_MSWIN
    hbounds = 3;
    vbounds = (Nlm_stdFontHeight * 3 / 2 - Nlm_stdLineHeight) / 2;
#endif
#ifdef WIN_MOTIF
    hbounds = 8;
    vbounds = 8;
