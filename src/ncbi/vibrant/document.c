...
} ListData, PNTR ListPtr;

typedef struct masterdata {
  ListPtr  list [MAXLISTS];
...
static Int2 GetItemNum (DocDataPtr ddatptr, Int2 desired)
  ...
  if (itemPtr != NULL && parsePtr != NULL && text != NULL && *text != '\0') {
    ...
    if (colFmt != NULL) {
      ...
      if (parsePtr->numCells > 0 && parsePtr->cellPtr != NULL) {
        ...
        str = (CharPtr) MemNew ((size_t) (len + 4 + minLines));
        if (str != NULL) {
          dst = str;
          for (row = 0; row < numRows; row++) {
            for (col = 0; col < numCols; col++) {
...

{
  ...
  if (itemPtr != NULL && itemPtr->text == NULL && itemPtr->prtProc != NULL) {
    ...
    if (text == NULL || *text == '\0') {
      ...
      itemPtr->neverCached = FALSE;
      needToParse = TRUE;
    } else if (ddatptr != NULL && ddatptr->virtual) {
      numRows = 0;
      ch = *text;
      while (ch != '\0') {
        if (ch == '\n') {
...
static Int2 GetStartsAt (DocDataPtr ddatptr, Int2 item)
...
static Int2 GetNumRows (DocDataPtr ddatptr, Int2 item)
...
static Int2 UpdateLineStarts (DocDataPtr ddatptr, Int2 frst)
...

{
  ...
  if (ddatptr != NULL) {
    ...
    if (itemPtr != NULL) {
      ...
      if (ddatptr->virtual && ddatptr->numItems > 0) {
...
static Int2 PixelsBetween (DocDataPtr ddatptr, RectPtr r, Int2 firstLine,
...

{
  ...
  if (d != NULL && item < 32767) {
    ...
    if (list < MAXLISTS && ddata.master != NULL) {
      ...
      listPtr = masterPtr->list [list];
      if (listPtr == NULL) {
        masterPtr->list [list] = MemNew (sizeof (ListData));
        listPtr = masterPtr->list [list];
      }
      if (listPtr != NULL) {
        index = (item % LISTSIZE);
...
*       Sets the document virtual scroll parameter
*
*****************************************************************************/

/* dgg -- "virtual" name conflicts w/ c++ syntax word -> isvirtual */
...

{
  ...
  if (ddata.master != NULL) {
    ...
    for (i = 0; i < MAXLISTS; i++) {
      masterPtr->list [i] = MemFree (masterPtr->list [i]);
    }
  }
