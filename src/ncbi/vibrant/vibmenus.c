...
static void Nlm_GetMenuItem (Nlm_GraphiC m, Nlm_Int2 item,
                             Nlm_CharPtr itemName, size_t maxsize)
...

{
  ...
  m = (Nlm_MenU) Nlm_GetParent ((Nlm_GraphiC) c);
  if (poplist) {
    itool = Nlm_AppendItems (m, (Nlm_IteM) c, title, POPUP_ITEM, FALSE);
  } else {
    itool = Nlm_AppendItems (m, (Nlm_IteM) c, title, CHOICE_ITEM, FALSE);
