...
CharPtr PersonIdPrint PROTO((PersonIdPtr pip, CharPtr buf));

void LIBCALLBACK PrintStackAddItem(AsnExpOptStructPtr aeosp); /* for building stack */
/*****************************************************************************
*
...
*
*****************************************************************************/
PrintTemplatePtr PrintTemplateFind (CharPtr name)
{
    Int2 i, PTnum;
...
*
*****************************************************************************/
Boolean PrintTemplateSetLoad ( CharPtr path )
{
    Char buf[80];
    AsnIoPtr aip;
	Boolean retval;
	...
	if (path == NULL)
	{
	    ...
	    StringCat(buf, "prt.prt");
		path = buf;
	}

    if ((aip = AsnIoOpen(path, "r")) == NULL)
	{
		ErrPost(CTX_NCBIOBJ, 1, "Couldn't open [%s]", path);
        return FALSE;
	}
...
*
*****************************************************************************/
PrintStackItemPtr PrintStackItemNew(PrintStackPtr psp)
{
...
*
*****************************************************************************/
PrintStackItemPtr PrintStackItemGet(PrintStackPtr psp, Int2 index, Int2Ptr indent_offset)
{
	...
	for (i = j; i < psp->pstackpos; i++)
	{
		...
		if (psip->pfip->template != NULL)  /* branch */
		{
			tmp = (PrintStackPtr)(psip->dv.ptrvalue);
			if ((count + tmp->size - 1) > index)  /* in here */
			{
				psip = PrintStackItemGet(tmp, (index - count), &local_offset);
...
*
*****************************************************************************/
Int2 PrintStackGetSize (PrintStackPtr psp)
{
	Int2 count = 0, i, tmp;
	PrintStackItemPtr psip;
...
*
*****************************************************************************/
PrintStackPtr PrintStackFree(PrintStackPtr psp)
{
	Int2 i;
	PrintStackItemPtr psip;
...
*
*****************************************************************************/
PrintFormatListPtr PrintFormatListBuild (PrintTemplatePtr ptp)
{
...
Int2 PrintFormatListGetMaxKey PROTO((PrintFormatListPtr pflp, Int2 start_key));
...
*
*****************************************************************************/
Int2 PrintFormatListGetMaxKey(PrintFormatListPtr pflp, Int2 start_key)
{
...
*
*****************************************************************************/
Int2 PrintFormatTraverse(PrintFormatPtr pfp, PrintFormatListPtr pflp, Int2 index, Int2 indent)
{
	PrintFormatItemPtr pfip;
	...
	while (pfp != NULL)
	{
		...
		if (pfp->form->choice == PrintForm_block)
		{
			...
			retcount = PrintFormatTraverse(pfbp->components, pflp, index, (indent+1));
...
*
*****************************************************************************/
void LIBCALLBACK PrintStackAddItem(AsnExpOptStructPtr aeosp)
{
	PrintFormatItemPtr pfip;
	...
	for (l = start_search; l < pslp->used; l++)
	{
		...
		for (i = 0; i < pflp->numitem; i++)
		{
			...
			if (! j)
			{
				if ((pfip->atp == atp) || (pfip->atp == base))
					got_one = TRUE;
			}
			else if ((pfip->types[j] == atp) && (type_indent >= j))
			{
			 	k = type_indent - 1;
				j--;
				while ((j > 0) && (k > 0))
				{
					if (pstack[k].type == pfip->types[j])
					{
						j--; k--;
					}
					else
				...
				{
					if ((pstack[k].type == pfip->types[0]) ||
						(AsnFindBaseType(pstack[k].type) == pfip->types[0]))
						got_one = TRUE;
				}
			}
			...
			if (got_one)
			{
				...
				if (aeosp->the_struct != NULL)
					...
					psip->dv = *(aeosp->dvp);
				if (pfip->template != NULL)  /* use template */
				{
					pflp1 = PrintFormatListGet(pfip->template->name);
					if (pflp1 == NULL)
					{
...
*
*****************************************************************************/
PrintStackPtr PrintStackBuild (Pointer data, AsnWriteFunc func, PrintFormatListPtr pflp)
{
	PrintStackPtr psp;
	AsnIoPtr aip;
	Boolean retval;
...
*
*****************************************************************************/
Boolean PrintStackSort (PrintStackPtr psp)
{
	PrintStackPtr tmp;
	PrintStackItemPtr psip;
	Int2 index;
	...
	Int2 index,	Int2 indent)
{
	...
	tmp = PrintStackGetLowest(src, (index + 1), (indent + 1), index, max_key, min_key);
	while (tmp >= 0)   /* copy/sort any components */
	{
		PrintStackSortNextItem(dest, src, tmp, (indent + 1));
		tmp = PrintStackGetLowest(src, (index + 1), (indent+1), tmp, max_key, min_key);
	}
...
Boolean PrintStringConvert PROTO((StdPrintOptionsPtr spop, CharPtr str, Boolean had_newline, Int2 indent, Boolean tilde));
/*****************************************************************************
*
*   PrintStackPrint(psp, fp)
*
*****************************************************************************/
void PrintStackPrint(PrintStackPtr psp, StdPrintOptionsPtr spop)
{
...
***/	
	if (psp->pflp->ptp->labelfrom != NULL)    /* find referred label */
	{
		...
		for (i = 0; i < psp->size; i++)
		{
			...
			if ((psip != NULL) && (! StringCmp(tmp, psip->pfip->format->asn1)))
			{
				...
				if ((label == NULL) && (pfip->template != NULL))
					label = pfip->template->format->label;
	...
	pfip = psip->pfip;
	if (pfip->template == NULL)
		form = pfip->format->form;
	else
		form = pfip->template->format->form;
...
***/
		...
		else                         /* popped the stack */
		{
			...
			while (oldindent >= curr_indent)
			{
				...
				if ((printed) || (printeds[oldindent]))
				{
					...
					if ((suffix == NULL) && (pfstack[oldindent]->template != NULL))
						suffix = pfstack[oldindent]->template->format->suffix;
					if (suffix != NULL)
					{
		...
		if (pfip->template != NULL)   /* use_template branch */
		{
			format = pfip->template->format;
		...
		if ((prefix == NULL) && (pfip->template != NULL))
			prefix = format->prefix;
		suffix = pfip->format->suffix;
		if ((suffix == NULL) && (pfip->template != NULL))
...
**/
		switch (form->choice)
		{
			...
			case PrintForm_block:
				...
				if ((i+1) < psp->size)
				{
					nxtpsip = PrintStackItemGet(psp, (i+1), &indent_offset);
...
void PrintCharFunc PROTO((Char c, StdPrintOptionsPtr spop));
/*****************************************************************************
*
*   Boolean PrintStringConvert(spop, str)
*
*****************************************************************************/
Boolean PrintStringConvert(StdPrintOptionsPtr spop, CharPtr str, Boolean had_newline, Int2 indent, Boolean tilde)
{
...
*
*****************************************************************************/
void PrintCharFunc(Char c, StdPrintOptionsPtr spop)
{
	if ((c < ' ') || (c > '~'))
	{
		if (c != '\n' && c != '\r' && c != '\t' && spop->fp != NULL)
...
*
*****************************************************************************/
Boolean StdFormatPrint (Pointer data, AsnWriteFunc func, CharPtr templatename, StdPrintOptionsPtr options)
{
	PrintStackPtr psp;
	CharPtr ptr = NULL;
	PrintFormatListPtr pflp;
...
PrintFormatListPtr PrintFormatListNew(void)
{
	PrintFormatListPtr pflp;
...
*
**************************************************/
PrintFormatListPtr PrintFormatListFree ( PrintFormatListPtr ptr)
{
	Int2 i;
...
*
*****************************************************************************/
void PrintFormatListFreeAll(Boolean templates)
{
    Int2 i;
...
*
*****************************************************************************/
PrintFormatListPtr PrintFormatListFind (CharPtr name)
{
    Int2 i;
...
*
*****************************************************************************/
PrintFormatListPtr PrintFormatListGet (CharPtr templatename)
{
	PrintFormatListPtr pflp;
...
*
*****************************************************************************/
StdPrintOptionsPtr StdPrintOptionsNew (FILE * fp)
{
...
*
*****************************************************************************/
StdPrintOptionsPtr StdPrintOptionsFree (StdPrintOptionsPtr spop)
{
...
*
*****************************************************************************/
CharPtr StdSeqLocPrint(DataValPtr dvp)
{
...
*
*****************************************************************************/
CharPtr StdDatePrint(DataValPtr dvp)
{
	Char buf[40];
...
*
*****************************************************************************/
CharPtr StdAuthListNamesPrint(DataValPtr dvp)
{
	AuthListPtr alp;
	Boolean 	first = TRUE;
	ValNodePtr   vnp;
...
*
*****************************************************************************/
CharPtr PersonIdPrint(PersonIdPtr pip, CharPtr buf)
{
	CharPtr tmp;
	NameStdPtr nsp;
	...
	if (pip->choice == 2)    /* std */
	{
		...
		if (nsp != NULL)
		{
			...
			else if (nsp->names[0] != NULL)             /* make from parts */
			{
				...
				else if (nsp->names [4] != NULL && nsp->names [0] != NULL)
				{
...
*
*****************************************************************************/
void PrintStackDump(PrintStackPtr psp, FILE *fp, Int2 indent)
{
	Int2 i, j;
	PrintStackItemPtr psip;
	...
	for (i = 0; i < psp->pstackpos; i++)
	{
		...
		if (psip->pfip->template != NULL)
		{
			...
			PrintStackDump((PrintStackPtr)(psip->dv.ptrvalue), fp, (indent+2));
		}
