                               ...
                               Uint1 strand, Boolean do_feat)
{
	...
	Int2 residue;
	ValNode fake;
	Int4 oldpos, tlen, tstart, tstop, tfrom, tto, xfrom, xto;
	Uint1 tstrand;
	SeqIdPtr tid;
	SeqLocPtr the_segs, currseg, head, curr, newhead, last, prev;
	SeqIntPtr sip;
		...
		(newbsp->repr == Seq_repr_ref))
	{
        if (newbsp->repr == Seq_repr_seg)  /* segmented */
		{
	        ...
	        the_segs = (SeqLocPtr)oldbsp->seq_ext;
		...
		if ((numloc) && (newbsp->repr == Seq_repr_ref))
		{
...
Boolean BioseqInsert (SeqIdPtr from_id, Int4 from, Int4 to, Uint1 strand, SeqIdPtr to_id, Int4 pos,
			Boolean from_feat, Boolean to_feat)
{
		...
		if (frombsp->repr == Seq_repr_map)
		{
			for (sfp = (SeqFeatPtr)(frombsp->seq_ext); sfp != NULL; sfp = sfp->next)
			{
				...
				if (newloc != NULL)   /* got one */
				{
					...
					newsfp->location = SeqLocInsert(newsfp->location, frombsp->id, 0,
				                                          pos, TRUE, to_id);
				}
			}
		}
	...
	if (to_feat)		     /* fix up sourceid Bioseq feature table(s) */
	{
		...
		while ((sfp = BioseqContextGetSeqFeat(bcp, 0, sfp, NULL, 1)) != NULL)
			sfp->product = SeqLocInsert(sfp->product, to_id, pos, len, TRUE, NULL);
