...
NI_InitServices(NI_DispatcherPtr disp, CharPtr user, CharPtr group, CharPtr password, NI_DispInfoPtr PNTR dip)
{
    ...
    if (status != 0)
        ErrPostEx(SEV_INFO,0,0,"WinSock 1.1 initialization failure, code %d", status-WSABASEERR);
    /* Try WinSock 1.1 and 1.0 in that order of preference */
    if (status != 0 && (status = WSAStartup(MAKEWORD(1,0),&wsaData)) != 0)
    {
        ErrPostEx(SEV_INFO,0,0,"WinSock 1.0 initialization failure, code %d", status-WSABASEERR);
    ...
    do {
        ...
        if ((disp->dispHP = DispatchConnect(disp, disp->dispHostName, disp->dispServiceName, disp->dispTimeout))
            ...
            HaltServices (disp);
            ErrPostEx(SEV_INFO,0,0, "NI_InitServices: Unable to connect to host <%s>, error <%s>", disp->dispHostName, ni_errlist[ni_errno]);
    ...
    if (disp->someBrokered)
    {
        ...
        { /* simulate service request by connecting to that port */
          ...
          RETRY:
            ...
            if ((status = connect(sHP->sok, (struct sockaddr PNTR) &serv_addr, sizeof(serv_addr))) < 0) { /* } */
#else
            if ((status = connect(sHP->sok, &serv_addr, sizeof(serv_addr))) < 0) {
                SOCK_ERRNO = ABS(status);
#endif
                switch (SOCK_ERRNO) {
                  case EINTR:
...
NI_ServerACK(void)
{
    ...
    { /* not stand-alone */
...
#endif /* NETP_INET_NEWT */
        { /* error */
            Message(MSG_FATAL, "Bind failed on socket %d, status = %d, errno = %d",
                    hp->sok, status, errno);
            return -1;
        }
        ...
        if ((sok = accept(hp->sok, (struct sockaddr *) &soktAddr, &soktLen)) < 0)
        { /* error */
            Message(MSG_FATAL, "Accept returned bad file descriptor %d, errno = %d",
                    sok, errno);
            return -1;
        }
        MsgDestroyHandle(hp);
        ...
        { /* security must be substring of client address */
            ...
            {
                close(sok);
                sleep(60); /* give the connection a chance to shutdown */
                Message(MSG_FATAL, "Security violation from IP address %s, security = %s\n",
...
NI_ServerNACK(CharPtr err_text)
{
    ...
    { /* not stand-alone */
        ...
        Message(MSG_FATAL, "Stand-alone server failed startup {%s}", temp_buf);
    }
    return 0;
} /* NI_ServerNACK */
