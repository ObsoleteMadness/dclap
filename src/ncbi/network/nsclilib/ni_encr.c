...
DoDesWrite(NI_EncrDataPtr encr, CharPtr buf, int len, CharPtr tmpbuf)
{
    ...
    MemSet ((CharPtr) encrBuf, '\0', encrLen);
 ...
 *              of the application's process ID.
 *
 * Note:
...
SetRandomSeed(void)
{
    ...
    if (! inited)
    {
        RandomSeed ((long) (GetSecs() | Nlm_GetAppProcessID()));
        inited = TRUE;
    }
}
 ...
 *              R_RANDOM_STRUCT data structure, and populate it with pseudo-random
 *              data.
 */
...
{
    R_RSA_PUBLIC_KEY PNTR retval = (R_RSA_PUBLIC_KEY PNTR) MemNew(sizeof(*retval));

    if (retval == NULL || internal == NULL || internal->modulus == NULL ||
