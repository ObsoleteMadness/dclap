...
*
*****************************************************************************/
AsnTypePtr LIBCALL  AsnBinReadId (AsnIoPtr aip, AsnTypePtr atp)
{
	Int4 used;
			...
			(aip->tagnumber == 0))     /* length 0, could be end of contents */
		{
			if (prevpsp->len == INDEFINATE)
			{
				...
				if (prevpsp->tag_indef)      /* indefinate user tag? */
				{
					...
					{
						AsnIoErrorMsg(aip, 3, prevpsp->type->name);
						return NULL;
					}
				}
				return prevpsp->type;
	...
	if (aip->type_indent)     /* check for SEQOF and SETOF */
	{
		...
		if ((isa == SEQOF_TYPE) || (isa == SETOF_TYPE))
		{
			...
			atp2 = AsnFindBaseType(atp);
			isa = atp2->type->isa;
			if (isa != CHOICE_TYPE)       /* have tags to read */
			{
	...
	if (atp == NULL)
	{
		AsnIoErrorMsg(aip, 5, prevpsp->type->name);
		return NULL;
	}
	...
	if (atp2->type->isa == SEQ_TYPE)      /* check sequence order */
	{
		...
		if (atp3 != NULL)
		{
			...
			while (atp4 != atp3)
			{
				if (atp == atp4)
				{
					AsnIoErrorMsg(aip, 6, atp->name, atp2->name);
					return NULL;
				}
				atp4 = atp4->next;
			}
		...
		while ((atp3 != NULL) && (atp3 != atp))
		{
			...
			{
				AsnIoErrorMsg(aip, 7, atp3->name, atp2->name);
				return NULL;
			}
			atp3 = atp3->next;
		}
...
*
*****************************************************************************/
Boolean AsnDeBinDecr (Int4 used, AsnIoPtr aip)
{
	PstackPtr	psp, psp0;
...
*
*****************************************************************************/
Int4 AsnDeBinScanTag (AsnIoPtr aip)
{
	Int4 used;
	Uint4 number;
	Int2 bytes, lenbytes;
...
*
*****************************************************************************/
Int2 LIBCALL  AsnBinReadVal (AsnIoPtr aip, AsnTypePtr atp, DataValPtr valueptr)
{
	Int4 used;
	Int2 isa, retval;
...
*
*****************************************************************************/
AsnTypePtr AsnDeBinFindType (AsnIoPtr aip, AsnModulePtr amp)
{
	AsnTypePtr atp;
...
*
*****************************************************************************/
AsnTypePtr AsnDeBinFindElement (AsnIoPtr aip, AsnTypePtr atp)
{
	while (atp != NULL)
	{
...
*
*****************************************************************************/
Int4 AsnDeBinReadInteger (AsnIoPtr aip, AsnTypePtr atp)
{
	Int2 bytes, len;
	Uint4 number;
	BytePtr bp;
	...
	AsnIoErrorMsg(aip, 12, value, atp->name);
...
*
*****************************************************************************/
Boolean AsnDeBinReadBoolean (AsnIoPtr aip, AsnTypePtr atp)
{
	BytePtr bp;
	Byte octet;
	Boolean result = TRUE;
...
*
*****************************************************************************/
CharPtr AsnDeBinReadString (AsnIoPtr aip, AsnTypePtr atp)
{
	Int4 len;
...
*
*****************************************************************************/
void AsnDeBinSkipString (AsnIoPtr aip)
{
	Int4 len;
	register int bytes, amount;
...
*
*****************************************************************************/
ByteStorePtr AsnDeBinReadOctets (AsnIoPtr aip)
{
	Int4 len;
	Int2 bytes, amount;
	BytePtr bp;
...
*
*****************************************************************************/
void AsnDeBinSkipOctets (AsnIoPtr aip)
{
	Int4 len;
	Int2 bytes, amount;
...
*
*****************************************************************************/
FloatHi AsnDeBinReadReal (AsnIoPtr aip)
{
	double result = 0.0;
	Int4 len;
	Int2 bytes, amount;
...
*
*****************************************************************************/
void AsnDeBinReadNull (AsnIoPtr aip, AsnTypePtr atp)
{
	aip->tagsaved = FALSE;
	if (aip->length != 0)
		AsnIoErrorMsg(aip, 16, atp->name);
...
*
*****************************************************************************/
void AsnDeBinSkipStruct (AsnIoPtr aip)
{
	aip->tagsaved = FALSE;
	AsnIoErrorMsg(aip, 0);
	return;
