...
*
*****************************************************************************/
void AsnOutput (CharPtr filename, AsnModulePtr amp, Boolean loader)
...
/*****************************************************************************
*
*   void AsnOutDefineType(fp, app, atp)
*
*****************************************************************************/
void AsnOutDefineType (FILE *fp, AsnProcPtr app, AsnTypePtr atp)

{
	char buf[100];

	AsnOutDefineElement(fp, app, atp, buf, NULL);
	return;
}

/*****************************************************************************
*
*   void AsnOutDefineElement(fp, app, atp, buf, pnt)
*
*****************************************************************************/
void AsnOutDefineElement (FILE *fp, AsnProcPtr app, AsnTypePtr atp, CharPtr buf, CharPtr pnt)

{
	...
	while (atp != NULL)
	{
		...
		if (atp->name != NULL)
		{
			...
			*to = '\0';
			...
			if (first)
				fprintf(fp, "\n");
			symbol = to;
			i = SYMBOLLEN_MAX;     /* max symbol length */
			while ((i) && (symbol > buf))
			{
				i--; symbol--;
			}
			if (symbol != buf)   /* longer than SYMBOLLEN_MAX */
			{
		...
		switch(atp->type->isa)
		{
			...
			case SET_TYPE:
			case CHOICE_TYPE:
				AsnOutDefineElement(fp, app, (AsnTypePtr) atp->branch, buf, to);
				break;
			case SEQOF_TYPE:
			case SETOF_TYPE:
				StringCpy(to, "_E");
				...
				switch (((AsnTypePtr)atp->branch)->type->isa)
				{
					...
					case SEQOF_TYPE:
					case SETOF_TYPE:
						AsnOutDefineElement(fp, app, (AsnTypePtr) atp->branch, buf, to+2);
						break;
					default:
						break;
				}
...
*
*****************************************************************************/
Int2 AsnOutFindValue (AsnProcPtr app, AsnValxNodePtr avnp)
...
*
*****************************************************************************/
AsnTypePtr AsnOutAddType (AsnProcPtr app, AsnTypePtr atp)
...
*
*****************************************************************************/
Boolean AsnOutNewType (AsnProcPtr app, AsnTypePtr atp)
...
*
*****************************************************************************/
AsnTmpTypePtr AsnOutFindType (AsnProcPtr app, AsnTypePtr atp)
...
*
*****************************************************************************/
void AsnOutNewValueChain (AsnProcPtr app, AsnValxNodePtr avnp)
