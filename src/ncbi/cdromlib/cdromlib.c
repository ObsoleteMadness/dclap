 ...
 * ===========================================================================
 *
 * File Name:  cdromlib.c
 * Author:   Greg Schuler, Jim Ostell, Jonathan Kans, Jonathan Epstein
 * Version Creation Date:   9-91
* $Revision: 2.31 $
 *
 * File Description: 
 *  Functions in this file are the I/O primitives needed to retrieve data
 *  from the "Entrez:Sequences" CD-ROM library produced by the NCBI.
 *
 *  NOTE:  Although file format specifications for the CD-ROM data files 
 *  will be made available upon request, it is strongly recommended that 
 *  applications use the functions in this file, rather than directly 
 *  reading the files.  This will provide some degree of insulation to
 *  future changes in the file formats.
 *
 * ==========================================================================
 */
...
static Boolean NEAR FieldDataAsnWrite PROTO((EntrezFieldDataPtr p, AsnIoPtr aip, AsnTypePtr orig));
static Boolean NEAR TypeDataAsnWrite PROTO((EntrezTypeDataPtr p, AsnIoPtr aip, AsnTypePtr orig, int num_fields));
...
Boolean    CdInitialize (CharPtr sCdRoot, CharPtr sVolume, CharPtr datvalpath, Int2Ptr ver)

{
    ...
    *ver = 0;
    ...
    if (vi->format != CURRENT_FORMAT_VERSION) {
        ErrPost(CTX_NCBICD, ERR_CD_BADFORMAT, sCdError[ERR_CD_BADFORMAT]);
        return FALSE;
    }  
...
/*  -------------------- CdSetPath() ---------------------------------
 */
Boolean   CdSetPath (Int2 dir, CharPtr path)

{
    ...
    if ((dir<2) || (dir>=NDIR) || (k==0)) {
		ErrPost(CTX_NCBICD, ERR_CD_BADDIR, sCdError[ERR_CD_BADDIR], dir);
        return FALSE;
    }
...

{
	...
	if ((fp = IdxFilePtr[type]) == NULL)
	{
		...
		{
			ErrPost(CTX_NCBICD, ERR_CD_FILEOPEN, sCdError[ERR_CD_FILEOPEN], path);
			return NULL;
		}
	}
	...
	if (j == 0)
	{
		ErrPost(CTX_NCBICD, ERR_CD_FILEREAD, sCdError[ERR_CD_FILEREAD]);
		return NULL;
	}
...

{
	...
	{
        FileClose (fd1);
		ErrPost(CTX_NCBICD, ERR_CD_FILEOPEN, sCdError[ERR_CD_FILEOPEN], filename);
        return  0;
    }
    ...
    while (cnt > 0)
	{
		...
		{
			ErrPost(CTX_NCBICD, ERR_CD_FILEWRITE, sCdError[ERR_CD_FILEWRITE]);
            break;
        }
        cntr -= cnt;
	    cnt = (size_t) MIN (cntr, (Int4)(BLKSIZE / sizeof(Int4)));
...

{
    if (((type < 0) || (type >= NTYPE)) && (type != TYP_SEQ)) {
		ErrPost(CTX_NCBICD, ERR_CD_BADTYPE, sCdError[ERR_CD_BADTYPE], type);
        return  FALSE;
    }
    return  TRUE;
}
...

{
    ...
    if (field<0 || field>=NFLD) {
		ErrPost(CTX_NCBICD, ERR_CD_BADFIELD, sCdError[ERR_CD_BADFIELD], field);
        return  FALSE;
    }
    if (vi->types[type].fields[field].num_bucket == 0) {
		ErrPost(CTX_NCBICD, ERR_CD_NOTERMS, sCdError[ERR_CD_NOTERMS], type, field);
        return  FALSE;
    }
    return  TRUE;
}
...

{
    ...
    if (vi->version != 0 || vi->issue != 0) { /* for compatibility with pre-release 6 data */
	  ...
	  {
        ErrPost (CTX_NCBICD, ERR_CD_BADINDEX, sCdError[ERR_CD_BADINDEX]);
		return FALSE;
      }
...

{
    ...
    buckets = vi->types[type].fields[field].num_bucket;
    bytes = (buckets + 1) * sizeof(CharPtr);
    if ((term_idx_str = (CharPtr PNTR) MemNew((size_t)bytes)) ==NULL) {
        FileClose (fd);
		ErrPost(CTX_NCBICD, ERR_CD_MEMORY, sCdError[ERR_CD_MEMORY]);
        return(0);
    }
...

{
    ...
    *p = '\0';        /* wipe-out the semi-colon */
    ...
    if (fp != NULL)
		...
		ErrPost(CTX_NCBICD, ERR_CD_FILEOPEN, sCdError[ERR_CD_FILEOPEN], (CharPtr) "cdromdat.val");
	MemFree(buf);
    return aip;
}
...
/*****************************************************************************
*
*   New Asn1 object support
*
*****************************************************************************/
/*****************************************************************************
*
*   EntrezInfoAsnRead(aip, type)
*
*****************************************************************************/
EntrezInfoPtr LIBCALL EntrezInfoAsnRead (AsnIoPtr aip, AsnTypePtr orig)

{
	...
	if (orig == NULL)           /* Cdrom-inf ::= (self contained) */
		atp = AsnReadId(aip, amp, CDROM_INF);
	else
		atp = AsnLinkType(orig, CDROM_INF);    /* link in local tree */
	oldtype = atp;

	AsnReadVal(aip, atp, &av);    /* read the START_STRUCT */

	cip = (EntrezInfoPtr) MemNew(sizeof(EntrezInfo));
	typectr = 0;
	fieldctr = 0;
	huff_left_ctr = 0;
	huff_right_ctr = 0;
	...
	while ((atp = AsnReadId(aip, amp, atp)) != oldtype)
	{
		...
		if (atp == CDROM_INF_types_E)
		{
			if (av.intvalue == START_STRUCT)
			{
				if (currtdp == NULL)
					currtdp = cip->types;
				else
					currtdp++;
				currtdp->fields = (EntrezFieldDataPtr) MemNew(sizeof(EntrezFieldData) * cip->field_count);
		...
		else if (atp == CDROM_INF_volume_label)
			cip->volume_label = (CharPtr)av.ptrvalue;
		else if (atp == CDROM_INF_version)
			cip->version = (Int2)av.intvalue;
		else if (atp == CDROM_INF_issue)
			cip->issue = (Int2)av.intvalue;
		else if (atp == CDROM_INF_format)
			cip->format = (Int2)av.intvalue;
		else if (atp == CDROM_INF_descr)
			cip->descr = (CharPtr)av.ptrvalue;
		else if (atp == CDROM_INF_no_compression)
			cip->no_compression = av.boolvalue;
		else if (atp == CDROM_INF_huff_count)
		{
			cip->huff_count = (Int2)av.intvalue;
			cip->huff_left = (Int2Ptr) MemNew(sizeof(Int2) * cip->huff_count);
			cip->huff_right = (Int2Ptr) MemNew(sizeof(Int2) * cip->huff_count);
		}
		else if (atp == CDROM_INF_huff_left_E)
		{
			cip->huff_left[huff_left_ctr] = (Int2)av.intvalue;
			huff_left_ctr++;
		}
		else if (atp == CDROM_INF_huff_right_E)
		{
			cip->huff_right[huff_right_ctr] = (Int2)av.intvalue;
			huff_right_ctr++;
		}
		else if (atp == CDROM_INF_type_count)
		{
			cip->type_count = (Int2)av.intvalue;
			cip->type_names = (CharPtr PNTR) MemNew(sizeof(CharPtr) * cip->type_count);
			cip->types = (EntrezTypeDataPtr) MemNew(sizeof(EntrezTypeData) * cip->type_count);
		}
		else if (atp == CDROM_INF_type_names_E)
		{
			cip->type_names[typectr] = (CharPtr)av.ptrvalue;
			typectr++;
		}
		else if (atp == CDROM_INF_type_bucket_size)
			cip->type_bucket_size = (Int2)av.intvalue;
		else if (atp == CDROM_INF_field_count)
		{
			cip->field_count = (Int2)av.intvalue;
		 	cip->field_names = (CharPtr PNTR) MemNew(sizeof(CharPtr) * cip->field_count);
		}
		else if (atp == CDROM_INF_field_names_E)
		{
			cip->field_names[fieldctr] = (CharPtr)av.ptrvalue;
			fieldctr++;
		}
		else if (atp == CDROM_INF_field_bucket_size)
			cip->field_bucket_size = (Int2)av.intvalue;
		else if (atp == TYPEDATA_num)
			currtdp->num = av.intvalue;
		else if (atp == TYPEDATA_num_uids)
			currtdp->num_uids = av.intvalue;
		else if (atp == TYPEDATA_minuid)
			currtdp->minuid = av.intvalue;
		else if (atp == TYPEDATA_maxuid)
			currtdp->maxuid = av.intvalue;
		else if (atp == TYPEDATA_num_bucket)
			currtdp->num_bucket = (Int2)av.intvalue;
		else if (atp == FIELDDATA_num_terms)
			currfdp->num_terms = av.intvalue;
		else if (atp == FIELDDATA_num_bucket)
			currfdp->num_bucket = (Int2) av.intvalue;
	}
	AsnReadVal(aip, atp, &av);    /* read the END_STRUCT */
...
*
*****************************************************************************/
EntrezInfoPtr LIBCALL EntrezInfoFree (EntrezInfoPtr cip)

{
	...
	MemFree(cip->volume_label);
	for (i = 0; i < cip->type_count; i++)
		MemFree(cip->types[i].fields);
	MemFree(cip->types);
	MemFree(cip->descr);
	MemFree(cip->huff_left);
	MemFree(cip->huff_right);
	if (cip->type_names != NULL)
	{
		for (i = 0; i < cip->type_count; i++)
			MemFree(cip->type_names[i]);
		MemFree(cip->type_names);
	}
	if (cip->field_names != NULL)
	{
		for (i = 0; i < cip->field_count; i++)
			MemFree(cip->field_names[i]);
		MemFree(cip->field_names);
	}
	return MemFree(cip);
}

/******************* Write funs for Cdrom-inf ********************************/

static Boolean NEAR FieldDataAsnWrite (EntrezFieldDataPtr p, AsnIoPtr aip, AsnTypePtr orig)

{
	...
	atp = AsnLinkType(orig, FIELDDATA); /* link local tree */
	...
	AsnWrite (aip, FIELDDATA_num_terms, &av);
	av.intvalue = p->num_bucket;
	AsnWrite (aip, FIELDDATA_num_bucket, &av);
...
static Boolean NEAR TypeDataAsnWrite (EntrezTypeDataPtr p, AsnIoPtr aip,
                                       AsnTypePtr orig, int num_fields)

{
	...
	atp = AsnLinkType(orig, TYPEDATA); /* link local tree */
	...
	if (! AsnStartStruct(aip, atp))
		goto erret;
	av.intvalue = p->num;
	AsnWrite (aip, TYPEDATA_num, &av);
	av.intvalue = p->num_uids;
	AsnWrite (aip, TYPEDATA_num_uids, &av);
	av.intvalue = p->minuid;
	AsnWrite (aip, TYPEDATA_minuid, &av);
	av.intvalue = p->maxuid;
	AsnWrite (aip, TYPEDATA_maxuid, &av);
	av.intvalue = p->num_bucket;
	AsnWrite (aip, TYPEDATA_num_bucket, &av);
	AsnStartStruct(aip, TYPEDATA_fields);
	for (i = 0; i < num_fields; i++)
	{
		FieldDataAsnWrite (&p->fields[i], aip, TYPEDATA_fields_E);
	}
	AsnEndStruct(aip, TYPEDATA_fields);
...
Boolean LIBCALL EntrezInfoAsnWrite (EntrezInfoPtr p, AsnIoPtr aip, AsnTypePtr orig)

{
	...
	atp = AsnLinkType(orig, CDROM_INF); /* link local tree */
	...
	if (! AsnStartStruct(aip, atp))
		goto erret;
	av.ptrvalue = p->volume_label;
	AsnWrite (aip, CDROM_INF_volume_label, &av);
	av.intvalue = p->version;
	AsnWrite (aip, CDROM_INF_version, &av);
	av.intvalue = p->issue;
	AsnWrite (aip, CDROM_INF_issue, &av);
	av.intvalue = p->format;
	AsnWrite (aip, CDROM_INF_format, &av);
	av.ptrvalue = p->descr;
	AsnWrite (aip, CDROM_INF_descr, &av);
	av.boolvalue = p->no_compression;
	AsnWrite (aip, CDROM_INF_no_compression, &av);

	av.intvalue = p->huff_count;
	AsnWrite (aip, CDROM_INF_huff_count, &av);
	AsnStartStruct (aip, CDROM_INF_huff_left);
	for (i = 0; i < p->huff_count; i++)
	{
		av.intvalue = p->huff_left[i];
		AsnWrite(aip, CDROM_INF_huff_left_E, &av);
	}
	AsnEndStruct (aip, CDROM_INF_huff_left);
	AsnStartStruct (aip, CDROM_INF_huff_right);
	for (i = 0; i < p->huff_count; i++)
	{
		av.intvalue = p->huff_right[i];
		AsnWrite(aip, CDROM_INF_huff_right_E, &av);
	}
	AsnEndStruct (aip, CDROM_INF_huff_right);

	av.intvalue = p->type_count;
	AsnWrite (aip, CDROM_INF_type_count, &av);
	AsnStartStruct (aip, CDROM_INF_type_names);
	for (i = 0; i < p->type_count; i++)
	{
		av.ptrvalue = p->type_names[i];
		AsnWrite(aip, CDROM_INF_type_names_E, &av);
	}
	AsnEndStruct (aip, CDROM_INF_type_names);

	av.intvalue = p->type_bucket_size;
	AsnWrite (aip, CDROM_INF_type_bucket_size, &av);

	av.intvalue = p->field_count;
	AsnWrite (aip, CDROM_INF_field_count, &av);
	AsnStartStruct (aip, CDROM_INF_field_names);
	for (i = 0; i < p->field_count; i++)
	{
		av.ptrvalue = p->field_names[i];
		AsnWrite(aip, CDROM_INF_field_names_E, &av);
	}
	AsnEndStruct (aip, CDROM_INF_field_names);

	av.intvalue = p->field_bucket_size;
	AsnWrite (aip, CDROM_INF_field_bucket_size, &av);

	AsnStartStruct (aip, CDROM_INF_types);
	for (i = 0; i < p->type_count; i++)
	{
		TypeDataAsnWrite (&p->types[i], aip, CDROM_INF_types_E,
						  p->field_count);
	}
	AsnEndStruct (aip, CDROM_INF_types);
...
*
*****************************************************************************/
CdTermPtr  CdTermRead (Int2 type, Int2 field, CharPtr ptr, CharPtr bufr, Int2 page)

{
	...
	*tmp2 = '\t';
	...
	trmptr->page = page + (Int2) (((size_t) (ptr - bufr)) / (size_t) BLKSIZE);
	return trmptr;
}
...
CdTermPtr  CdTermFree (CdTermPtr trmp)
...

{
	...
	*result = NULL;
	...
	for (; j < numuid; j++)
	{
		...
		{
			MemFree(newuids);
			newuids = (DocUidPtr) MemNew((size_t)(sizeof(DocUid) * (count + 1)));
   ...
   do
   {
       if ((r - l) > 15)
       {
           ...
           if(i)
           {
               ...
               if (scnt >= 100)
				{
					ErrPost(CTX_NCBICD, ERR_CD_MEMORY, "linksort > 100");
					return;
				}
           }
           else
...
static Int2 LIBCALLBACK DecompReadFunc (Pointer p, CharPtr buff, Uint2 count)
{
	...
	if (dcp->compr == COMPR_DONT_KNOW)
	{
		...
		if (dcp->compr == COMPR_NONE)
		{
			...
			{
				ErrPost(CTX_NCBICD, ERR_CD_BADDECOMP,
