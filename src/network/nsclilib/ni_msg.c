...
Int2 
NI_DestroyMsgNack(NINackPtr mp)
{
    if (mp == NULL)
        return 1;
    if (mp->reason != NULL)
...
Int2 
NI_DestroyMsgLogin(NILoginPtr lp)
{
    if (lp == NULL)
        return 1;
    if (lp->uid != NULL)
...
Int2 
NI_DestroyMsgSvclist(NISvcListPtr sp)
{
    if (sp == NULL)
        return 1;
    if (sp->toolset != NULL)
...
Int2 
NI_DestroyMsgSvcreq(NISvcReqPtr sp)
{
    if (sp == NULL)
        return 1;
    if (sp->uid != NULL)
...
Int2 
NI_DestroyMsgSvcresp(NISvcRespPtr sp)
{
    if (sp == NULL)
        return 1;
    if (sp->request != NULL)
...
Int2 
NI_DestroyMsgPreResp(NIPreRespPtr cp)
{
    if (cp == NULL)
        return 1;
    MemFree(cp);
...
Int2 
NI_DestroyMsgAcct(NIAcctPtr ap)
{
    if (ap == NULL)
        return 1;
    if (ap->jobname != NULL)
...
Int2 
NI_DestroyMsgCatalog(NICatalogPtr cp)
{
    NodePtr             np;
...
Int2 
NI_DestroyRegion(NIRegionPtr rp)
{
    if (rp == NULL)
        return 1;
...
int
NI_ReadPubKey(AsnIoPtr extaip, AsnTypePtr extatp, NIPubKeyPtr pubkey)
{
    AsnIoPtr      savaip = aip;
...
void
NI_WritePubKey(AsnIoPtr extaip, AsnTypePtr extatp, NIPubKeyPtr pubkey)
{
    AsnIoPtr savaip = aip;
    AsnTypePtr savatp = atp;
...
int 
NI_ReadDispInfo(AsnIoPtr extaip, AsnTypePtr extatp, NIDispInfoPtr dip)
{
    AsnIoPtr     savaip = aip;
    AsnTypePtr   savatp = atp;
    DataVal      value;
...
void
NI_WriteDispInfo(AsnIoPtr extaip, AsnTypePtr extatp, NIDispInfoPtr dip)
{
    AsnIoPtr savaip = aip;
    AsnTypePtr savatp = atp;
...
Int2 
NI_DestroyToolset(NIToolsetPtr tsp)
{
    NodePtr             np;
...
NIToolsetPtr 
NI_GetCatToolset(NIToolsetPtr tsp)
{
    NIToolsetPtr        dtsp;
    NISvcPtr            svcp, tsvcp;
    NIResPtr            resp, tresp;
...
NIMsgPtr 
MsgRead(MHandPtr hp, Boolean unblocked)
{
    DataVal     value;
    NIMsgPtr    mp = NULL;
...
Int2 
MsgWrite(MHandPtr hp, NIMsgPtr mp, Boolean noDestroy)
{
    DataVal     value;
