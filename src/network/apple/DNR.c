...
/*----------------------------------------------------------
	...
	This file started life on the CW DR/3 CD. It was crap.
	I had to hack it to death to get it to work.
...
typedef struct hostInfo* HostInfoPtr;
typedef OSErr (*StrToAddrProcPtr)(long selector, char* hostName, HostInfoPtr rtnStruct,
...
char *userDataPtr;
{
	...
	err= CallStrToAddrProc(dnr, STRTOADDR, hostName, rtnStruct, resultproc, userDataPtr);
	return(err);
}
...
typedef struct returnRec *ReturnRecPtr;
typedef OSErr (*HInfoProcPtr)(long selector, char* hostName, ReturnRecPtr returnRecPtr,
								long resultProc, char* userData);
...
char *userDataPtr;
{
	...
	err= CallHInfoProc(dnr, HINFO, hostName, returnRecPtr, resultProc, userDataPtr);
	return(err);
}
