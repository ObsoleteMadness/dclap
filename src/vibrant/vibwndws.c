...
/* currently only used in WIN_MOTIF, window creation */
char  Nlm_appname[256];

#ifdef WIN_MOTIF

/* 
     user can override these w/ .Xresources entry in form of
  appname*resourcename: xxx
-- or --
  Vibrant*resourcename: xxx
*/
static String colorXresources[] = {
  ...
  "*Background:                         #ffffffffffff", /*cfcfcfcfcfcf = grey*/
...
{
#ifndef DCLAP
	/* !! dclap doesn't want to be redrawing all over the place like this does ! */
  Nlm_Update ();
#endif
  if (w != NULL) {
    Nlm_UseWindow (w);
...
#ifdef DCLAP
	/* general protection against too small window sizes */
...
#ifdef DCLAP
	/* we want to stick app name here, thru some new vibrant global var */  
  shl = XtAppCreateShell (Nlm_appname, (String) "Vibrant",
...
/* static -- dgg, make this public */
void Nlm_ResizeWindow (Nlm_GraphiC w, Nlm_Int2 dragHeight,
...

{
  ...
  */
  if (g != NULL && notInside) {
    top = NULL;
    while (g != NULL) {
      thisitem = (Nlm_RevPtr) Nlm_MemNew (sizeof (Nlm_RevItem));
      if (thisitem != NULL) {
        thisitem->graphic = g;
        thisitem->next = top;
        top = thisitem;
...

{
  ...
  if (buf != NULL && maxsize > 0) {
#ifdef DCLAP
		if (*gProgramPath) {
			Nlm_StringNCpy (buf, gProgramPath, maxsize);
			return;
			}
#endif
    ...
    block.ioWDIndex = 0;
    block.ioWDProcID = 0;
    PBGetWDInfo (&block, FALSE); /* ! this won't work after a StdOpen or other WD changer !! */
  ...
  PtoCstr ((StringPtr) directory);
    ...
    if (ptr [0] == DIRDELIMCHR) {
      Nlm_StringNCpy (buf, statargv [0], maxsize);
			...
			/* this will FAIL for program run from PATH !! - dgg*/
      ptr = statargv [0];
...
#endif
#ifdef DCLAP
	if (buf[0]) Nlm_StringNCpy(gProgramPath, buf, PATH_MAX);
#endif
  }
}
...
#ifdef DCLAP
				// 68k mac app is bombing here now (not before) ... ?? why
			if (StrNCmp((char*)&Nlm_currentEvent.where.v,"oapp",4)==0) break;
#endif
      AEProcessAppleEvent (&Nlm_currentEvent);
      break;
    default:
      break;
  }
...

{
  ...
  if (hwndCtl == NULL) {
#ifdef DCLAP
		if (!Nlm_menuHwnd) Nlm_menuHwnd= Nlm_GetWindowPtr(Nlm_theWindow);
#endif
...

{
  ...
  switch (message) {
    ...
    case WM_CHAR:
#ifdef DCLAP
			/* Nlm_cmmdKey= (((unsigned long)lParam >> 29) & 1); /* alt key flag */
#endif
...
#ifdef DCLAP
  Nlm_StringNCpy (Nlm_appname, appname, sizeof (Nlm_appname));
#endif
...
{
#if defined(WIN_MAC) && (defined(COMP_THINKC) || defined(COMP_CODEWAR))
  KeyMap  keys;
  ...
  if ((keys [1] & 1) != 0) {
...
#endif
#ifdef WIN_MAC
	/* dgg hack for powerpc */
#if defined(powerc) || defined(__powerc)
	strcpy( (Nlm_CharPtr)apName+1, "noname"); apName[0]= 6; 
	apRefNum= 0;
	apParam= NULL;
	{
	OSErr err;
	...
	if ( err == 0 ) {
		...
		info.processName = (StringPtr) &apName;
		info.processAppSpec = &fileSpec;
		err= GetProcessInformation( &psn, &info);
		if (err == 0) {
			/* memcpy( apName, info.processName, info.processName[0]); */
			apRefNum= fileSpec.vRefNum; 

			/* !! we need the following for apRefNum */
			err= OpenWD( fileSpec.vRefNum, fileSpec.parID, 0, &wdref);
			apRefNum= wdref;
			/* err= CloseWD( wdref); /* ?? */
			}
		}
...
#ifndef DCLAP
	/* dgg -- speed up quit; all this cleanup isn't needed */
  Nlm_CleanUpWindows ();
...

{
  Nlm_GraphiC  prev;
  Nlm_GraphiC  prnt;
  Nlm_GraphiC  thisitem;

  prev = NULL;
  if (g != NULL) {
    prnt = Nlm_GetParent (g);
    thisitem = Nlm_GetChild (prnt);
    while (thisitem != NULL && thisitem != g) {
      prev = thisitem;
      thisitem = Nlm_GetNext (thisitem);
    }
  }
  return prev;
}
