...
#define INT short
	...
	enum {
		AF_INET,SOCK_STREAM, SOL_SOCKET, SO_LINGER, SO_REUSEADDR, SO_KEEPALIVE
		};
	...
	unsigned long inet_addr( char* hostname)
	{
		return 0;
	}

	short connect(short sockfd, SockAddPtr Server, size_t serversize)
	{
		return 0;
	}

	short socket( short AF_INET, short SOCK_STREAM, short val)
	{
		return 0;
	}

	short s_close(long theSocket)
	{
		return 0;
	}

	short s_write(long itsSocket, void* buffer, long buflen) 
	{
		return 0;
	}

	void setsockopt(short sockfd, short SOL_SOCKET,  short SO_LINGER, short x, short y)
	{
	...
	short gethostname( char* name, short namelen)
	{
		return 1;
	}
	...
	typedef struct hostent *hostPtr1;

	hostPtr1 gethostbyname( char* hostname)
	{
		return (hostPtr1) 0;
	}

	short getsockname( short itsSocket, SockAddPtr name, long* len)
	{
		if (len) *len= 0;
		return 0;
	}

	short s_read(short itsSocket, void* buffer, size_t buflen)
	{
		return 1;
	}
	...
	typedef struct timeval *timeptr;

	short select( short numsocks, fd_set* readsocks, 
				  fd_set* writesocks, fd_set* errsocks, timeptr time)
  {
		return 0;
	}
...
{
	return select( numsocks, (fd_set*)readsocks, (fd_set*)writesocks, (fd_set*)errsocks, (struct timeval *) time);
}
