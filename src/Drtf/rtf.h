 ...
 */
/* dgg added _RTF_ and __cplusplus flag */
...
extern short	rtfParam;		/* control symbol parameter */
...
#if 0
	/* dgg -- don't want this for dclap -- doesn't handle font styles properly */
# define	rtfReadCharSet		0x00	/* auto-read charset files */
# define	rtfSwitchCharSet	0x00	/* auto-switch charset maps */
#else
...
struct RTFStyleElt
{
	...
	short		rtfSEParam;	/* control symbol parameter */
...
typedef	void (*RTFFuncCharPtr) (char *);		/* generic function pointer */
...
void 	RTFSetClassCallback (short , RTFFuncPtr callback);

RTFFuncPtr	RTFGetClassCallback (short aclass);
...
short	RTFCheckCM (short aclass, short major);
short	RTFCheckCMM (short aclass, short major, short minor);
...
# define	RTFAlloc(size)	_RTFAlloc ((short) size)
char * _RTFAlloc (short size);
...
void		RTFSetMsgProc (RTFFuncCharPtr proc);
void		RTFSetPanicProc (RTFFuncCharPtr proc);
...
typedef FILE * (*LibFileOpen) (char* file, char* mode);

void RTFSetOpenLibFileProc (LibFileOpen proc);
